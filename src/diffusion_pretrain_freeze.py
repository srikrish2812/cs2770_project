...
def get_args():
    ...
    parser.add_argument("--output_dir", type=str, default="../checkpoints/diffusion_pretrain_freeze")
...
class DiffusionViT(nn.Module):
    ...
    def forward_encoder(self, noisy_tokens, timesteps):
        ...
        hidden_states = encoder_outputs.last_hidden_state if hasattr(encoder_outputs, 'last_hidden_state') else encoder_outputs[0]  # [B, 577, 1024]
...
def train(args):
    # Create output directory (append freeze info)
    if args.freeze_layers > 0:
        args.output_dir = args.output_dir.rstrip("/") + f"_{args.freeze_layers}"
    ...
    # -----------------------------------------------------------------
    # Freeze early encoder layers (professor's suggestion #1)
    # -----------------------------------------------------------------
    if args.freeze_layers > 0:
        frozen_count = 0
        for name, param in diffusion_vit.named_parameters():
            should_freeze = False

            # Always keep denoising_head and timestep_embed trainable
            if name.startswith("denoising_head.") or name.startswith("timestep_embed."):
                should_freeze = False

            # Freeze embeddings (Conv2d, position embedding, class embedding, pre_layrnorm)
            elif name.startswith("embeddings.") or name.startswith("pre_layrnorm."):
                should_freeze = True

            # Freeze early encoder layers (0 to freeze_layers-1)
            elif name.startswith("encoder.layers."):
                layer_num = int(name.split(".")[2])
                if layer_num < args.freeze_layers:
                    should_freeze = True
        ...
        trainable_params = sum(p.numel() for p in diffusion_vit.parameters() if p.requires_grad)
        logger.info(f"Layer freezing: layers 0-{args.freeze_layers-1} frozen, "
                    f"layers {args.freeze_layers}-23 trainable")
        logger.info(f"Frozen params:    {frozen_count / 1e6:.1f}M")
        ...
        for name, param in diffusion_vit.named_parameters():
            if param.requires_grad:
                if name.startswith("encoder.layers."):
                    trainable_components.add(f"encoder.layers.{name.split('.')[2]}")
                else:
                    trainable_components.add(name.split(".")[0])
        logger.info(f"Trainable components: {sorted(trainable_components)}")
    else:
        trainable_params = sum(p.numel() for p in diffusion_vit.parameters() if p.requires_grad)
        logger.info(f"All layers trainable (freeze_layers=0)")
        logger.info(f"Trainable params: {trainable_params / 1e6:.1f}M")
    ...
    # -----------------------------------------------------------------
    if args.freeze_layers > 0:
        # When freezing early layers, Conv2d is frozen too.
        # Just optimize all remaining trainable params at the same LR.
        trainable_param_list = [p for p in diffusion_vit.parameters() if p.requires_grad]
        optimizer = torch.optim.AdamW(
            trainable_param_list,
            lr=args.lr,
            weight_decay=args.weight_decay,
        )
        logger.info(f"Optimizer: AdamW, LR={args.lr} (all trainable params, Conv2d frozen)")
    else:
        # Original behavior: differential LR for Conv2d vs rest
        ...
        logger.info(f"Optimizer: AdamW, Conv2d LR={args.lr * args.conv_lr_scale:.1e}, ViT LR={args.lr:.1e}")
    ...
    logger.info(f"Freeze layers: {args.freeze_layers} of 24")
    ...
    logger.info(f"Frozen layers: 0-{args.freeze_layers-1} | Trainable layers: {args.freeze_layers}-23" if args.freeze_layers > 0 else "All layers trainable")

    diffusion_vit.train()

    # Set frozen layers to eval mode (disables dropout in frozen layers)
    if args.freeze_layers > 0:
        diffusion_vit.embeddings.eval()
        diffusion_vit.pre_layrnorm.eval()
        for i in range(args.freeze_layers):
            diffusion_vit.encoder.layers[i].eval()
        ...
        for step, pixel_values in enumerate(train_loader):
            ...
            if (step + 1) % args.grad_accum_steps == 0:
                ...
                torch.nn.utils.clip_grad_norm_(
                    [p for p in diffusion_vit.parameters() if p.requires_grad],
                    args.max_grad_norm
                )
            ...
            if (step + 1) % args.log_every == 0:
                ...
                if len(optimizer.param_groups) > 1:
                    ...
                    lr_str = f"LR: conv={lr_conv:.2e} vit={lr_vit:.2e}"
                else:
                    lr_str = f"LR: {lr_current:.2e}"
